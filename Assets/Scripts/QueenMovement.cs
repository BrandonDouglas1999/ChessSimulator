...
public class QueenMovement : Piece
{
    public QueenMovement(int piece, int x, int y, GameObject gameObject) : base(piece, x, y, gameObject)
    {
    }

    public override bool canMove(int x, int y)
    {
        if ((base.x == x ^ base.y == y) && !isBlockedMovement(x, y)) //if same horizontal/vertical
        {
            return true;
        }
        else
    ...
    {
        if ((base.x == x ^ base.y == y) && !isBlockedCapture(x, y)) //if same horizontal/vertical
        {
            return true;
        }
        else
